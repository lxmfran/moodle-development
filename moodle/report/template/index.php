...
/**
 * A report to display the outcome of scheduled template
 *
 * @package    report
 * @subpackage template
...
admin_externalpage_setup('reporttemplate', '', null, '', array('pagelayout'=>'report'));
...
if ($courseid) {
    ...
    // Get the automated template that have been performed for this course.
    $params = array('operation' => backup::OPERATION_BACKUP,
                    'type' => backup::TYPE_1COURSE,
                    'itemid' => $course->id,
                    'interactive' => backup::INTERACTIVE_NO);
    if ($template = $DB->get_records('backup_controllers', $params, 'timecreated DESC',
        'id, backupid, status, timecreated', $page, 1)) {
        // Get the backup we want to use.
        $backup = reset($template);
        ...
        if ($logs = $DB->get_records('backup_logs', array('backupid' => $backup->backupid), 'timecreated ASC',
            ...
            foreach ($logs as $log) {
                $table->data[] = array(userdate($log->timecreated, get_string('strftimetime', 'report_template')) .
                    ' - ' . $log->message);
            }
        } else {
            $table->data[] = array(get_string('nologsfound', 'report_template'));
        }
    }
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('backupofcourselogs', 'report_template', $coursename));
    if (isset($backup)) {
        // We put this logic down here as we may be viewing a backup that was performed which there were no logs
        // recorded for. We still want to display the pagination so the user can still navigate to other template,
        // and we also display a message so they are aware that the backup happened but there were no logs.
        $baseurl = new moodle_url('/report/template/index.php', array('courseid' => $courseid));
        $numberoftemplate = $DB->count_records('backup_controllers', $params);
        $pagingbar = new paging_bar($numberoftemplate, $page, 1, $baseurl);

        echo $OUTPUT->render($pagingbar);
        echo $OUTPUT->heading(get_string('logsofbackupexecutedon', 'report_template', userdate($backup->timecreated)), 3);
        echo html_writer::table($table);
        echo $OUTPUT->render($pagingbar);
    } else {
        echo $OUTPUT->box(get_string('notemplatefound', 'report_template'), 'center');
    }
    echo $OUTPUT->footer();
    exit();
}
...
foreach ($rs as $backuprow) {
    ...
    $backuplogsurl = new moodle_url('/report/template/index.php', array('courseid' => $backuprow->courseid));
    $backuplogsicon = new pix_icon('t/viewdetails', get_string('viewlogs', 'report_template'));
    $cells = array(
...
$automatedtemplateenabled = get_config('backup', 'backup_auto_active');

// Display the backup report
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string("backuploglaststatus"));
echo $OUTPUT->box_start();
if (empty($automatedtemplateenabled)) {
    // Automated template aren't active, display a notification.
    // Not we don't stop because of this as perhaps scheduled template are being run
    // automatically, or were enabled in the page.
    echo $OUTPUT->notification(get_string('automatedtemplateinactive', 'backup'));
}
echo html_writer::table($table);
echo $OUTPUT->box_end();
echo $OUTPUT->footer();
